...
var ModalProvider = function (props) {
    ...
    var _e = react_1.useState(''), currentModal = _e[0], setCurrentModal = _e[1];
    var _f = react_1.useState(false), oneIsOpen = _f[0], setOneIsOpen = _f[1];
    var _g = react_1.useState(false), closeOnBlur = _g[0], setCloseOnBlur = _g[1];
    var _h = react_1.useState(false), bodyScrollIsLocked = _h[0], setBodyScrollIsLocked = _h[1];
    var _j = react_1.useState(''), cssString = _j[0], setCSSString = _j[1];
    var _k = react_1.useState(''), classPrefix = _k[0], setClassPrefix = _k[1];
    var closeAll = react_1.useCallback(function (updateHistory) {
        ...
        if (updateHistory) {
            ...
            if (typeof handleParamChange === 'boolean' && handleParamChange) {
                ...
                var newURL = "" + window.location.pathname + queryWithoutModal;
    ...
    var bindEsc = react_1.useCallback(function (e) {
        if (e.keyCode === 27)
            closeAll();
    }, [closeAll]);
    ...
    react_1.useEffect(function () {
        if (shouldGenerateCSS) {
            var newString = '';
            newString = css_1.default(classPrefix, zIndex);
            if (minifyCSS)
                newString = newString.replace(/\n/g, '').replace(/\s\s+/g, ' ');
            setCSSString(newString);
        }
    }, [shouldGenerateCSS, minifyCSS, zIndex, classPrefix]);
    var open = react_1.useCallback(function (slug, skipParamChange) {
        if (!skipParamChange) {
            if (typeof handleParamChange === 'function') {
                handleParamChange({
                    key: 'modal',
            ...
            if (typeof handleParamChange === 'boolean' && handleParamChange) {
                ...
                var newURL = "" + window.location.pathname + queryWithModal;
                window.history.pushState({}, '', newURL);
            }
        }
    ...
    var toggle = react_1.useCallback(function (slug) {
        if (slug === currentModal)
            closeAll();
        else
            open(slug);
    }, [closeAll, open, currentModal]);
    var setBodyScrollLock = react_1.useCallback(function (shouldLock, excludingRef) {
        if (shouldLock) {
            body_scroll_lock_1.disableBodyScroll(excludingRef.current);
            setBodyScrollIsLocked(true);
        }
        else {
            body_scroll_lock_1.enableBodyScroll(excludingRef.current);
            setBodyScrollIsLocked(false);
        }
    }, []);
    var setContainerRef = react_1.useCallback(function (ref) {
    ...
    return (react_1.default.createElement(react_1.Fragment, null,
        ...
        react_1.default.createElement(ModalContext_1.default.Provider, { value: __assign(__assign({}, inheritedProps), { transTime: transTime,
                ...
                bodyScrollIsLocked: bodyScrollIsLocked,
                ...
                setBodyScrollLock: setBodyScrollLock }) }, children && children)));
};
